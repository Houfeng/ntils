 ...
 * @static
 */
export function replace(str: string, str1: string, str2: string) {
  if (isNull(str)) return str;
 ...
 * @static
 */
export function startWith(str1: string, str2: string) {
 ...
 * @static
 */
export function contains(str1: string, str2: string) {
  if (isNull(str1) || isNull(str2)) return false;
 ...
 * @static
 */
export function endWith(str1: string, str2: string) {
  if (isNull(str1) || isNull(str2)) return false;
 ...
 * @static
 */
export function has(obj: Object, name: string) {
 ...
 * @static
 */
export function each(list: any, handler: Function, scope?: any) {
 ...
 * @static
 */
export function formatDate(date: any, format: string, dict?: any) {
  if (isNull(format) || isNull(date)) return date;
  date = toDate(date);
  dict = dict || {};
  const placeholder: { [key: string]: number } = {
  ...
  for (let key in placeholder) {
    if (new RegExp("(" + key + ")").test(format)) {
      ...
      let repalceValue = RegExp.$1.length == 1 ?
        value : ("00" + value).substr(("" + value).length);
      format = format.replace(RegExp.$1, repalceValue as string);
    }
  }
  return format;
}
 ...
 * @static
 */
export function copy(src: any, dst: any, igonres?: any) {
  dst = dst || (isArray(src) ? [] : {});
  each(src, function (key: any) {
 ...
 * @static
 */
export function clone(src: any, igonres: any) {
  if (isNull(src) ||
    isString(src) ||
    isNumber(src) ||
    isBoolean(src) ||
  ...
  each(src, function (key: any, value: any) {
 ...
 */
export function mix(
  dst: any, src: any, igonres?: any, mode?: any, igonreNull?: any
): any {
 ...
 * 定义不可遍历的属性
 **/
export function final(obj: any, name: any, value: any): any {
  if (arguments.length < 1) throw new Error('Parameter missing');
  if (arguments.length < 2) {
    return each(obj, function (name: any, value: any) {
...
export function deepEqual(a: any, b: any) {
  ...
  var result = true;
  each(allKeys, function (i: any, key: any) {
...
export function fromTo(
  fromNum: any, toNum: any, step: any, handler: any
) {
 ...
 * 对象变换
 **/
export function map(list: any, fn: any) {
  const buffer: any = isArray(list) ? [] : {};
  each(list, function (name: any, value: any) {
...
export function setByPath(obj: any, path: any, value: any) {
  ...
  each(path, function (index: any, name: any) {
...
export function unique(array: any) {
  ...
  each(array, function (i: any, value: any) {
...
/**
  ...
  */
export function toCamelCase(str: string, mode?: number) {
  if (!isString(str)) return '';
  if (str) {
...
export function parseHTML(str: string) {
  ...
  // win._NPH_.innerHTML = '';
  each(childNodes, function (index: any, childNode: any) {
